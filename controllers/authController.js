...
const register = async (req, res) => {
    ...
    res.status(StatusCodes.CREATED).json({roomNumber, department, building, role});
};
...
const login = async (req, res) => {
    ...
    if(!roomNumber || !department || !password){
        throw new BadRequestError("Please provide the roomNumber, department and the office's password to login");
    }
    const office = await Office.findOne({roomNumber});

    if(!office){
        throw new NotFoundError("The office with this room number or department does not exists");
    }
    ...
    attachCookiesToResponse(res, officePayload);
    res.status(StatusCodes.OK).json({roomNumber: office.roomNumber, department: office.department, building: office.building, role: office.role});
};
